...
void Motor::setSpeed(int speed) {
    if (speed == 0) {
        ...
        int duty_cycle = constrain(speed, PWM_FWD_MIN, PWM_MAX);  // min power needed for forward motion adjust '25'
        // Invert the pwm duty cycle
        duty_cycle = PWM_MAX - duty_cycle;      // 0 - 255 means 255 - 0 that is HIGH PWM duty means lower speed
        ledcWrite(pwm_channel_, duty_cycle);  // was duty_cycle
    } 
    else {
        // Set direction to reverse
        digitalWrite(dir_pin_, LOW);
        int duty_cycle = constrain(-speed, PWM_REV_MIN, PWM_MAX);  // min power needed for reverse motion adjust '130'
        // Invert the pwm duty cycle
        ledcWrite(pwm_channel_, duty_cycle);  // was duty_cycle
    }
}
